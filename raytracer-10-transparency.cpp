...
```bash
g++ raytracer-10-transparency.cpp -o main.out -std=c++20 -Ofast
./main.out
open output.bmp
```
...
struct Object {
    ...
    Object(const float3& v_color, float v_specular, float v_reflective, float v_refractive_index = -1) {
        color = v_color;
        specular = v_specular;
        reflective = v_reflective;
        refractive_index = v_refractive_index;
    }
...
struct Sphere : Object {
    ...
    Sphere(const float3& v_center, float v_radius, const float3& v_color, float v_specular, float v_reflective, float v_refractive_index)
    : Object(v_color, v_specular, v_reflective, v_refractive_index) {
        center = v_center;
        radius = v_radius;
...
struct BoundingSphere : Sphere {
    ...
    BoundingSphere(Object* v_object, const float3& v_center, float v_radius, const float3& v_color, float v_specular, float v_reflective, float v_refractive_index) 
    : Sphere(v_center, v_radius, v_color, v_specular, v_reflective, v_refractive_index) {
        object = v_object;
    }
...
// NOTE: Does not account for transparent objects.
...
float3 trace_ray(
    ...
    float refractive_index, // default is 1, for air
    Scene scene
) {
    ...
    const float3 reflected_color = trace_ray(intersection.point, reflected_ray, EPSILON, INFINITY, recursion_depth - 1, 1, scene);
...
int32_t main() {
    ...
    // Define scene
    std::vector<Object*> objects = {
        new Sphere({0, -1, 3}, 1, {255, 0, 0}, 500, 0.2, -1),
        new Sphere({-2, 0, 4}, 1, {0, 255, 0}, 10, 0.4, -1),
        new Sphere({2, 0, 4}, 1, {0, 0, 255}, 500, 0.3, -1),
        new Sphere({0, -5001, 0}, 5000, {255, 255, 0}, 1000, 0.5, -1),
        new BoundingSphere(
            new CSG(
                new Sphere({0, 0, 2}, 0.5, {0, 0, 0}, 0, 0, -1),
                new Sphere({0, 0, 3}, 0.5, {0, 0, 0}, 0, 0, -1),
                AND, {0, 0, 0}, 0, 0
            ),
            {0, 0, 2.5}, 0.5, {255, 255, 255}, 0, 0, 1.33
        ),
    };
    ...
    for (int32_t x = -width / 2; x < width / 2; x++) {
        for (int32_t y = -height / 2; y < height / 2; y++)
        {
            ...
            float3 color = trace_ray(camera.position, direction, 1, INFINITY, 3, 1, scene);
